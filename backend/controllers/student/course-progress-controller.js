...
const getVideoUrl = async (req, res) => {
  try {
    ...
    // Verify if user has purchased the course
    const studentPurchasedCourses = await StudentCourses.findOne({ userId });
    ...
    // Get the course to find the lecture's public_id
    const course = await Course.findById(courseId);
    if (!course) {
      return res.status(404).json({
        success: false,
    ...
    const lecture = course.curriculum.id(lectureId) || course.curriculum.find(l => l._id.toString() === lectureId);
